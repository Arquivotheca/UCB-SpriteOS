/* Compiler driver program that can handle many languages.
 ...
 $Header: /sprite/src/benchmarks/itc/gcc/RCS/gcc.c,v 1.2 93/02/11 17:12:42 kupfer Exp $ SPRITE (Berkeley)
...
or with constant text in a single argument.
 ...
 %C	process target->compile_spec as a spec.  This gives target-specific
	switches for the C compiler, including the name of the compiler.
 %a     process target->asm_spec as a spec.
        This allows specs.h to specify part of the spec for running the
	assembler, including the name of the assembler.
 %l     process target->link_spec as a spec.  Spec must start with name
	of linker.
 %L     process target->lib_spec as a spec.
 %m	substitute the official name of the target machine (may be
	different than the name specified in the -m switch, if there are
	several unofficial names for the same machine).
 %S     process target->startfile_spec as a spec.  A capital S is actually
	used here.
