/* $Header: /sprite/src/benchmarks/itc/sld/RCS/sld.c,v 1.2 93/02/11 17:21:44 kupfer Exp $ SPRITE (Berkeley) */
...
static char *rcsid = "$Header: /sprite/src/benchmarks/itc/sld/RCS/sld.c,v 1.2 93/02/11 17:21:44 kupfer Exp $ SPRITE (Berkeley)";
      ...
      * Name is the name of the library for error messages.  */
{
	...
	!arMemberNameIs(&SYMDEFHead, "__.SYMDEF") ||
...
bool
arMemberNameIs(hdr, name)
    struct ar_hdr *hdr;
    char *name;
    /* Decide whether the given header refers to a file with the given 
     * name.  This is non-trivial because of differing encoding schemes 
     * used by different versions of ar. */
{
    int nameLength = strlen(name);

    if (strncmp(hdr -> ar_name, name, nameLength) != 0) {
	return FALSE;
    }
    if (hdr -> ar_name[nameLength] == ' ') {
	return TRUE;			/* old BSD format */
    } else if (hdr -> ar_name[nameLength] == '/') {
	return TRUE;			/* SunOS 5.x */
    }
