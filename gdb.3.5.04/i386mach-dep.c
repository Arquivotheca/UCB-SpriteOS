 ...
 * Revision 1.1  90/09/07  16:53:42  roy
 * Initial revision.
 * 
 * Revision 3.3  90/06/04  17:47:06  kupfer
 * Fix so that we correctly recognize the start of a frame with lots of
 * locals.
 * 
 * Revision 3.2  90/05/25  16:31:11  kupfer
 * Lint.  Get text load address from system header file.  Be (slightly)
 * smarter about getting registers from a core dump.
 * read_inferior_memory must return an error code if passed a bogus
 * address.  Fix "info float" (would make gdb drop core).  Tweaks.  Fix
 * i386_frame_num_args to deal with frameless functions.
 * 
 * Revision 3.1  90/05/15  17:24:03  kupfer
 * gdb 3.5 from CMU.
 * 
	...
	thread_t th;
{
  ...
  if ((ret = thread_get_state(th, i386_THREAD_STATE, &state, &stateCnt))
      != KERN_SUCCESS) 
      if (ret!=MIG_ARRAY_TOO_LARGE)
	      return 0;
...
/* XXX - should really examine the header.  ld now accepts an option
   to set the load address. */
#ifndef N_TXTADDR
#define N_TXTADDR(hdr) USRTEXT
#endif /* no N_TXTADDR */
     ...
     int from_tty;
{
  ...
  if (filename)
	...
	/* The register array is stored at the top of the kernel
	   stack.  See i386/pcb.c in the kernel sources. */
	/* reg_offset = ctob(UPAGES) - I386_SAVED_REGISTERS * 4; */
        /* I386_SAVED_REGISTERS not defined.  Use the previous
           constant: 19 */
	reg_offset = ctob(UPAGES) - 19 * 4;
	  ...
	  for (regno = 0; regno < NUM_REGS; regno++)
	    {
	      ...
	      val = lseek (corechan, i386_register_u_addr (reg_offset, regno),
			   0);
	      if (val < 0)
		perror_with_name (filename);
     ...
     int peek_flag;
{
  ...
  if (read_memory (codestream_addr, (unsigned char *)codestream_buf,
		   sizeof (int))
      != 0)
    error("Can't read code stream at 0x%x.", codestream_addr);
...
/* symbols like 'EAX' come from <i386/reg.h> */
...
#if DUMB_STDIO
      if (is_nan(&val, sizeof(val)))
	printf("  NaN\n");
      else if (is_inf(&val, sizeof(val)))
	printf("  Infinity\n");
      else
	printf("  %g\n", val);
#else /* DUMB_STDIO */
     ...
     char *buf;
{
  ...
  kern_return_t ret;
  unsigned int stateCnt = i386_FLOAT_STATE_COUNT;
...
/* Return number of args passed to a frame.
   Can return -1, meaning no way to tell.  This can happen if gdb gets
   slightly confused by a frameless function. */
     ...
     struct frame_info *fi;
{
  ...
  if (pfi == NULL)
    return -1;				/* outermost call frame, i.e. main() */
  retpc = pfi->pc;
  if (read_memory (retpc, &op, sizeof(op)) != 0)
    return -1;				/* can't tell */
  if (op == 0x59)			/* pop %ecx */
    numargs = 1;
