 ...
 * Revision 1.3  92/01/09  16:36:47  kupfer
 * In validate_frame(), invalidate the entire rest of the cache, instead
 * of just one element.  This fixes a problem where stack backtraces were
 * getting truncated when debugging multi-threaded programs.
 * 
 * Revision 1.2  91/10/29  15:57:17  kupfer
 * Don't define VM_MIN_ADDRESS if it's already defined.
 * 
 * Revision 1.1  91/10/29  13:04:39  kupfer
 * Initial revision
 * 
 * Revision 1.1  90/09/07  16:56:27  roy
 * Initial revision.
 * 
	...
	struct frame_saved_regs *frp;
{
	...
	if (!ci->valid)
	  error("couldn't find valid frame");
...
/* Debugging routine to dump the cache of frame information for the current 
   thread. */ 
...
dump_frame_cache()
{
  ...
  printf("Frame cache:\n");
  for (ci = &chain_info[0]; ci; ci = ci->next)
    {
      printf("%sVALID: frame=0x%x, pc=0x%x, ra=0x%x\n",
	     ci->valid ? "" : "IN", ci->the_frame, ci->the_pc, ci->the_ra);
    }
  printf("End frame cache.\n");
}
