     ...
     char *filename;
{
  ...
  for (ps = partial_symtab_list; ps; ps = ps->next)
    {
      s = (struct symtab *)ps->filename;
      fprintf (outfile, "Partial Symtab x%x for %sfile %s\n\n", ps,
      	(ps->readin ? "readin " : ""),
      	(ps->readin ? s->filename : ps->filename));
      fprintf (outfile, "range x%x..x%x\n",
        ps->textlow, ps->texthigh);
      fprintf (outfile, "ldsymoff x%x, ldsymlen x%x\n",
        ps->ldsymoff, ps->ldsymlen);
      fprintf (outfile, "%d dependencies: ", ps->number_of_dependencies);
      {
	register int i;
	for (i = 0; i < ps->number_of_dependencies; i++)
	  fprintf (outfile, "x%x ", ps->dependencies[i]);
      }
      fprintf (outfile, "\n");
      fprintf (outfile, "%d globals at off x%x\n",
      	ps->n_global_syms, ps->globals_offset);
      fprintf (outfile, "%d statics at off x%x\n",
      	ps->n_static_syms, ps->statics_offset);
      fprintf (outfile, "\n\n");
/* XXX should print globals and statics here XXX */
    }
...
_initialize_symmisc ()
{
  ...
  add_com ("printpsyms", class_obscure, print_psymtabs,
	   "Print dump of partial symtabol tables to file OUTFILE.");
}
