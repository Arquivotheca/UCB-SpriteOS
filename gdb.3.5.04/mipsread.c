...
/*
 * $Log:	mipsread.c,v $
 * Revision 1.3  91/11/13  22:17:37  kupfer
 * Merge in Sandro's bug fix for .c files that are included by other .c
 * files.
 * 
 */
...
#define UNSAFE_TEXT_ADDR(p)	((unsigned)(p) < SAFE_TEXT_ADDR || (unsigned)(p) > 2*SAFE_TEXT_ADDR)
#define UNSAFE_ADDR(p)	(UNSAFE_DATA_ADDR(p) && UNSAFE_TEXT_ADDR(p))
	...
	HDRR *hdr;
{
	...
	/* The array (of lists) of globals must be sorted.
	   ...
	   WELL.... not exactly.  I got stung by the case foo.c included
	   ../foo.c, grrrrr.  Anyways, since texthigh is calculated,
	   and based on the start address of the next-high-up file,
	   we'll fixup later the screwups. */
	...
	*prevhigh = END_OF_TEXT_SEGMENT(filhdr);
	...
	/* Now for the fixup we mentioned above.. */
	{
		register struct partial_symtab *ps, *ns;
		for (ps = partial_symtab_list; ps; ps = ns) {
			/* end of list ? */
			if ((ns = ps->next) == 0) continue;

			/* texthigh == 0 means no code here, don't bother */
			if (ps->texthigh == 0) continue;
			while (ns && ns->texthigh == 0) ns = ns->next;
			if (ns == 0) continue;

			if (ps->texthigh > ns->textlow) {
printf("Right-adjusting %s[x%x..x%x] and %s[x%x..x%x]\n",
ps->filename, ps->textlow, ps->texthigh,
ns->filename, ns->textlow, ns->texthigh);
				ps->texthigh = ns->textlow;
			}
			if (ps->texthigh < ps->textlow) {
printf("Left-adjusting %s[x%x..x%x] and %s[x%x..x%x]\n",
ps->filename, ps->textlow, ps->texthigh,
ns->filename, ns->textlow, ns->texthigh);
				ps->texthigh = ns->textlow;
			}
	...
	char **pname;
{
	...
	if (rf == -1) {
		...
		*pname = (UNSAFE_ADDR(sh->iss)) ? "<undefined>" :
			(char *) sh->iss;

		/* Have we parsed it ? */
		if ((!UNSAFE_ADDR(sh->value)) && (sh->st == stParsed)) {
