...
/*
 * $Log:	findvar.c,v $
 * Revision 1.1  92/01/08  18:25:14  kupfer
 * Initial revision
 * 
 * Revision 1.1  90/09/07  16:53:14  roy
 * Initial revision.
 * 
 * Revision 3.2  90/05/25  16:37:19  kupfer
     ...
     char *myaddr;
{
  ...
  if (addr)
	...
	if (read_memory (addr, myaddr, REGISTER_RAW_SIZE (regnum)) != 0)
	  error("Can't read register at 0x%x.", addr);
      return;
    }
     ...
     int regnum;
{
  ...
  if (addr)
	...
	return value_from_long (builtin_type_int, (LONGEST) addr);
      if (read_memory (addr, raw_buffer, REGISTER_RAW_SIZE (regnum)) != 0)
	error("Can't read register at 0x%x.", addr);
    }
  else
...
#define REG_STRUCT_HAS_ADDR(gcc_p) 0
  ...
  if (read_memory (addr, VALUE_CONTENTS (v), len) != 0)
    error("Can't read variable at 0x%x.", addr);
  ...
  if (num_storage_locs > 1)
	    ...
	    ++local_regnum))
	{
	  ...
	  if (addr == 0)
	    ...
	    {
	      if (read_memory (addr, value_bytes + value_bytes_copied,
			       REGISTER_RAW_SIZE (local_regnum))
		  != 0)
		error("Can't read register at 0x%x.", addr);
	      mem_stor++;
