...
static int TypeToReg(RegType reg, DecodedInstruction instr)
{
    switch (reg) {
      case RS:
	return instr.rs;
...
//
// If there is any kind of exception, we return immediately,
// allowing the instruction to be re-executed -- on a syscall,
...
Machine::OneInstruction()
{
    ...
    // Record any delayed load -- new values for loadReg and loadValue
    int nextLoadReg = 0, nextLoadValue = 0;

    ASSERT(machine->pageTable != NULL);
    // Fetch instruction 
    if (!machine->pageTable->ReadMem(registers[PCReg], 4, &raw)) 
	return;					// some kind of error
    
    DecodedInstruction instr(raw);	// decode instruction
	...
	if (!machine->pageTable->ReadMem(tmp & ~0x3, 4, &value))
	    return;
	...
	if (!machine->pageTable->ReadMem(tmp, 4, &value))
	    return;
#ifdef BIG_ENDIAN
	if (!(tmp & 0x2))
	    value >>= 16;
	...
	break;
      ...
      case OP_SB:
	if (!machine->pageTable->WriteMem((unsigned) 
	...
	abort();
#endif
	if (!machine->pageTable->WriteMem((tmp & ~0x3), 4, value))
	    return;
	break;
    ...
    registers[0] = 0;		 // Make sure R0 stays zero.
...
DecodedInstruction::DecodedInstruction(unsigned int value)
{
    OpInfo *opPtr;
