...
#endif
/*
 * This is the backwards compatible version of Net_RouteInfo.
 */
...
extern ReturnStatus	Net_StringToAddr _ARGS_((char *buffer, int protocol,
						 ...
						 Net_AddressOld *addressPtr));
extern char		*Net_AddrToString _ARGS_((Net_AddressOld *netAddressPtr,
						  int protocol,
						  Net_NetworkType netType,
						  char *buffer));
#else
...
extern ReturnStatus	Net_StringToAddrNew _ARGS_((char *buffer, 
				Net_AddressType type, Net_Address *addressPtr));
extern char		*Net_InetAddrToString _ARGS_((Net_InetAddress address,
						      char *buffer));
extern char		*Net_AddrToStringNew _ARGS_((Net_Address *netAddressPtr,
						char *buffer));
...
#ifdef NEW_NET
/*
 * These are only needed for backwards compatibility.  They can be deleted 
 * once the "new" routines become the normal ones.
 */

#define Net_AddrToString Net_AddrToStringNew
#define Net_StringToAddr Net_StringToAddrNew
#endif
