...
#ifndef lint
static char rcsid[] = "$Header: /sprite/src/lib/tcl/RCS/tclBasic.c,v 1.86 91/01/29 10:44:02 ouster Exp $ SPRITE (Berkeley)";
#endif
				 ...
				 * by a previous call to Tcl_CreateInterp). */
{
    ...
    iPtr->commandPtr = NULL;
    TclDeleteVars(iPtr, iPtr->globalPtr);
    if (iPtr->events != NULL) {
...
#   define NUM_ARGS 10
	...
	while (1) {
		...
		case ']': {
		    if ((openBraces == 0) && !openQuote && (termChar == ']')) {
			goto cmdComplete;
		    }
		    *dst = ']'; dst++;
		    break;
	...
	if (!(iPtr->flags & ERR_IN_PROGRESS)) {
	    sprintf(copyStorage, ", while executing\n\"%.*s%s\"",
	...
	} else if (syntaxPtr == first) {
	    sprintf(iPtr->result, "%s: '=> %.*s'", syntaxMsg,
		    last-syntaxPtr, syntaxPtr);
	} else {
    ...
    char *message;		/* Message to record. */
{
     ...
     * new errorInfo is the concatenatioin of the message in
     * interp->result and the new message.
     */

    if (iPtr->flags & ERR_IN_PROGRESS) {
	prefix = Tcl_GetVar(interp, "errorInfo", 1);
	if (prefix == NULL) {
	    prefix = "";
	}
    } else {
	prefix = interp->result;
	iPtr->flags |= ERR_IN_PROGRESS;
    }
 ...
 * ArgEnd --
 *
 *	This procedure is invoked by the parser to see if we're
 *	currently at the end of an argument in the input.
 *
 * Results:
 *	1 is returned if the characters at *p terminate an argument.
 *	0 is returned if they form a continuing part of an ongoing
 *	argument
 *
 * Side effects:
 *	None.
...
static int
ArgEnd(p, termChar)
    char *p;			/* Points to characters that may or may
				 * not be at the end of an argument. */
    char termChar;		/* Character that terminates commands. */
{
    register char c = *p;
...
#define FIXED_SIZE 200
     ...
     * the commands gets too large than call malloc to create the
     * space.
     */
	...
	if ((spaceUsed + length) > spaceAvl) {
	    ...
	    spaceAvl += spaceAvl/2;
	    new = malloc((char *) spaceAvl);
	    bcopy(cmd, new, spaceUsed);
	    if (cmd != fixedSpace) {
		free(cmd);
	    }
	    cmd = new;
