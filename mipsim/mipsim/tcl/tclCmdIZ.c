...
#ifndef lint
static char rcsid[] = "$Header: /sprite/src/lib/tcl/RCS/tclCmdIZ.c,v 1.43 90/07/25 08:55:30 ouster Exp $ SPRITE (Berkeley)";
#endif
...
#   define ARG_SIZE 20
	...
	sprintf(iPtr->result, "bad \"%.20s\" option \"%.20s\": must be args, body, commands, cmdcount, default, exists, globals, level, locals, procs, tclversion, or vars",
		argv[0], argv[1]);
	return TCL_ERROR;
    }
    ...
    char **argv;			/* Argument strings. */
{
	...
	if (strncmp(argv[2], "chars", strlen(argv[2])) != 0) {
		sprintf(interp->result,
		    "bad argument \"%.50s\":  must be \"chars\"",
		    argv[2]);
	    return TCL_ERROR;
	}
	count = strlen(p);
    ...
    char **argv;			/* Argument strings. */
{
	...
	if ((argc != 3) && (argc != 5)) {
	    sprintf(iPtr->result,
		    "wrong # args: should be \"%.50s variable name [ops command]\"",
		    argv[0]);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    /*
	     * Give back information about current trace, if any.
	     */

	    char *((*procPtr)());
	    TraceVarInfo *wvPtr;
	    int flags;
	    char *result[2], ops[4], *p;

	    flags = Tcl_VarTraceInfo(interp, argv[2], 0, &procPtr,
		    (ClientData *) &wvPtr);
	    if ((flags == 0) || (procPtr != TraceVarProc)) {
		return TCL_OK;
	    }
	    p = ops;
	    if (wvPtr->flags & TCL_TRACE_READS) {
		*p = 'r';
		p++;
	    }
	    if (wvPtr->flags & TCL_TRACE_WRITES) {
		*p = 'w';
		p++;
	    }
	    if (wvPtr->flags & TCL_TRACE_DELETES) {
		*p = 'd';
		p++;
	    }
	    *p = '\0';
	    result[0] = ops;
	    result[1] = wvPtr->command;
	    iPtr->result = Tcl_Merge(2, result);
	    iPtr->dynamic = 1;
	    return TCL_OK;
	}
	if (argv[4][0] == '\0') {
	    /*
	     * Delete current trace, if it's us rather than someone else.
	     */

	    char *((*procPtr)());
	    ClientData clientData;

	    (void) Tcl_VarTraceInfo(interp, argv[2], 0, &procPtr, &clientData);
	    if (procPtr == TraceVarProc) {
		Tcl_UnTraceVar(interp, argv[2], 0);
	    }
	    return TCL_OK;
	} else {
		...
		if (*p == 'r') {
		    ...
		    flags |= TCL_TRACE_DELETES;
		} else {
		    badOps:
		    sprintf(interp->result,
			    "%s \"%.50s\": must be combination of rwd",
			    "bad operations", argv[3]);
		    return TCL_ERROR;
		}
	    }
	    if (flags == 0) {
	    ...
	    wvPtr = (TraceVarInfo *) malloc((unsigned)
		    (sizeof(TraceVarInfo) + strlen(argv[4])));
	    wvPtr->flags = flags;
	    flags |= TCL_TRACE_DELETES;
	    strcpy(wvPtr->command, argv[4]);
	    if (Tcl_TraceVar(interp, argv[2], 0, flags, TraceVarProc,
		    (ClientData) wvPtr) != TCL_OK) {
		free((char *) wvPtr);
		return TCL_ERROR;
	    }
	    return TCL_OK;
	...
	sprintf(iPtr->result, "bad option \"%.20s\": must be variable",
		argv[1]);
	return TCL_ERROR;
    }
}
...
static char *
TraceVarProc(wvPtr, interp, name, global, flags, oldValue, newValue)
    TraceVarInfo *wvPtr;	/* Information about the variable trace. */
    Tcl_Interp *interp;		/* Interpreter containing variable. */
    char *name;			/* Name of variable. */
    int global;			/* 1 means variable is global, 0 means local. */
    int flags;			/* Indicates operation being performed and
				 * state of variable. */
    char *oldValue;		/* Current value of variable. */
    char *newValue;		/* Requested new value (if this is a write
				 * operation). */
{
    char *argv[5];
    char *fullCommand, opString[3], *result;
    int code;
    Interp dummy;

    result = NULL;
    if (wvPtr->flags & flags) {

	/*
	 * Generate a Tcl command to handle this access.
	 */
    
	argv[0] = wvPtr->command;
	argv[1] = name;
	if (flags & TCL_TRACE_READS) {
	    opString[0] = 'r';
	} else if (flags & TCL_TRACE_WRITES) {
	    opString[0] = 'w';
	} else if (flags & TCL_TRACE_DELETES) {
	    opString[0] = 'd';
	}
	if (flags & TCL_VARIABLE_UNDEFINED) {
	    opString[1] = 'u';
	    opString[2] = '\0';
	} else {
	    opString[1] = '\0';
	...
	argv[3] = oldValue;
	argv[4] = newValue;
	fullCommand = Tcl_Merge(5, argv);
    
	/*
	 * Execute the command, being careful to save and restore the
	...
	if (interp->dynamic) {
	    ...
	    strcpy(dummy.resultSpace, interp->result);
	    dummy.dynamic = 0;
	}
	code = Tcl_Eval(interp, fullCommand, 0, (char **) NULL);
	free(fullCommand);
	if ((code == TCL_OK) && !(flags & TCL_TRACE_DELETES)) {
	    Tcl_SetVar(interp, name, interp->result, global);
	}
	if (dummy.dynamic) {
	    Tcl_Return(interp, dummy.result, TCL_DYNAMIC);
	} else {
	    Tcl_Return(interp, dummy.resultSpace, TCL_VOLATILE);
	}
	if ((code == TCL_OK) || !(flags & TCL_TRACE_READS)) {
	    result = Tcl_GetVar(interp, name, global);
	}
    }
    if (flags & TCL_TRACE_DELETES) {
	free((char *) wvPtr);
    }
    return result;
}
