...
#ifndef lint
static char rcsid[] = "$Header: /sprite/src/lib/tcl/RCS/tclGet.c,v 1.4 91/01/21 15:48:02 ouster Exp $ SPRITE (Berkeley)";
#endif /* not lint */
    ...
    int *intPtr;		/* Place to store converted result. */
{
    ...
    if ((end == string) || (*end != 0)) {
	sprintf(interp->result, "expected integer but got \"%.50s\"",
		string);
	return TCL_ERROR;
    }
    *intPtr = i;
    ...
    double *doublePtr;		/* Place to store converted result. */
{
	...
	sprintf(interp->result,
		"expected floating-point number but got \"%.50s\"",
		string);
	return TCL_ERROR;
    }
    *doublePtr = d;
				 ...
				 * will be 0 or 1. */
{
	...
	*boolPtr = 0;
	...
	sprintf(interp->result, "expected boolean value but got \"%.50s\"",
		string);
	return TCL_ERROR;
    }
    return TCL_OK;
