 ...
 *
 * $Header: /sprite/src/lib/tcl/RCS/tclInt.h,v 1.28 90/07/25 08:55:24 ouster Exp $ SPRITE (Berkeley)
 */
...
typedef struct Var {
    ...
    struct Var *globalPtr;	/* For variables declared "global" within
				 * a tclProc, this points to the global
				 * variable corresponding to name.
				 * Otherwise it is NULL. */
    char *((*traceProc)());	/* Procedure to monitor reads and/or writes
				 * to variable.  NULL means no trace
				 * established for this variable.  Should
				 * only be non-null if globalPtr == NULL. */
 ...
 * Variable flags:
 *
 * TCL_TRACE_READS:	1 means that traceProc should be called to
 *			handle all reads of variable.
 * TCL_TRACE_WRITES:	1 means that traceProc should be called to
 *			handle all writes to variable.
 * TCL_TRACE_DELETES:	1 means that traceProc should be called when
 *			the trace is deleted.
 * TCL_VARIABLE_UNDEFINED:
 *			1 means this variable has not yet been assigned
 *			a value (e.g. it's been declared global or a trace
 *			has been set for it before it is assigned to).
 * The above flags are all defined in tcl.h.  Be careful not to
 * re-use their values below!
 *
 * VAR_DYNAMIC:		1 means the storage space for the value was
 *			dynamically allocated, and must eventually be
 *			freed.
 * VAR_TRACE_ACTIVE:	1 means that the traceProc is currently active
 *			for this variable;  to avoid infinite operations,
 *			any operations on the variable should act as if
 *			there were no trace.
 */

#define VAR_DYNAMIC		32
#define VAR_TRACE_ACTIVE	64
...
/*
 * Library imports;  needed because not all systems have all the
 * right declarations in all the right places.
 */
